...
gdp.growth.summary <- data.frame(gdp.growth.summary.names, gdp.growth.summary.values)
gdp.growth.summary
```
...
```{r}
# library(MASS)
# gdp.growth.fit <- fitdistr(gdp.worldbank.subset$gdp_growth, "lognormal")
# gdp.growth.hist + 
#   stat_function(fun=dlnorm, 
#                 colour = "green", 
#                 size = 1, 
#                 args=list(meanlog=log(mean(gdp.worldbank.subset$gdp_growth)), 
#                           sdlog=log(sd(gdp.worldbank.subset$gdp_growth))))
```
...
```{r}
country.data.merged <- merge(gdp.worldbank.subset, rural.pop.wide, by.x = "Country", by.y = "country", incomparables = NULL)

View(head(country.data.merged))
```

Next we'll add a new variable to the merged data frame to get the change in percent population from 2011 to 2012.
```{r}
country.data.merged$rural_pop_growth <- country.data.merged$rural_pop_percent_2012 - country.data.merged$rural_pop_percent_2011
```
